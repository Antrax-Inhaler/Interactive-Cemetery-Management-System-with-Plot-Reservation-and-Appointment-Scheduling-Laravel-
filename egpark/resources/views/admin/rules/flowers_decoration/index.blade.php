...
h5{
    ...
    <section class="home-section" style="width: calc(100% - 58px); overflow: scroll">
        <h1>Flowers & Decoration Rules</h1>
        <button class="custom-btn custom-btn-primary" onclick="openModal('addModal')">Add New Rule</button>
        ...
        <table>
            ...
            <tbody>
                @foreach ($flowersDecorations as $rule)
                    <tr>
                        <td>{{ $rule->id }}</td>
                        <td>{{ $rule->description }}</td>
                        <td>
                            <button class="custom-btn custom-btn-warning" onclick="openEditModal('{{ $rule->id }}', '{{ $rule->description }}')">Edit</button>
                            <form action="{{ route('admin.flowers_decoration.destroy', $rule->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
        ...
        <div id="addModal" class="modal">
            <div class="modal-content">
                <form action="{{ route('admin.flowers_decoration.store') }}" method="POST">
                    @csrf
        ...
        <script>
            ...
            function openEditModal(id, description) {
                ...
                document.getElementById('editForm').action = '/admin/flowers_decoration/' + id;
                openModal('editModal');
            }
