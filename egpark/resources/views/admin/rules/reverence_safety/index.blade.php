    ...
    <div class="home-content" style="display: block;">
        <div class="panel">
            ...
            <table>
                ...
                <tbody>
                    @foreach ($reverenceSafeties as $rule)
                        <tr>
                            <td>{{ $rule->id }}</td>
                            <td>{{ $rule->description }}</td>
                            <td>
                                <button class="custom-btn custom-btn-warning" onclick="openEditModal('{{ $rule->id }}', '{{ $rule->description }}')">Edit</button>
                                <form action="{{ route('admin.reverence_safety.destroy', $rule->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
            ...
            <div id="addModal" class="modal">
                <div class="modal-content">
                    <form action="{{ route('admin.reverence_safety.store') }}" method="POST">
                        @csrf
...
<script>
    ...
    function openEditModal(id, description) {
        ...
        document.getElementById('editForm').action = '/admin/reverence_safety/' + id;
        openModal('editModal');
    }
