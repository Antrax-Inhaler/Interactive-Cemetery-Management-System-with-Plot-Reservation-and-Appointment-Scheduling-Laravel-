@include('admin.sidenav')  <!-- Updated to include admin side navigation -->
<meta name="csrf-token" content="{{ csrf_token() }}">

<section class="home-section" style="width: calc(100% - 58px); overflow: scroll;">
    <div class="home-content" style="display: block;">
        <div class="panel">
            <h1 style="text-align: left;">Admin Notifications</h1><br>  <!-- Updated heading -->
            <hr><br>
                ...
                <div class="notification-list-container">
                    <div class="notification-list">
                        @foreach ($notifications as $notification)
                            <div class="notification-item {{ $notification->is_read_admin ? '' : 'unread' }}" 
                                 data-id="{{ $notification->id }}"
                                 data-type="{{ $notification->type }}"
                                 onclick="viewNotificationDetails({{ $notification->id }}, '{{ $notification->type }}')">
                                <p>From: <strong>{{ $notification->user->name }}</strong> {{ $notification->admin_message }}</p> <!-- Display user name -->
                            </div>
                        @endforeach
                    </div>
                </div>
...
<script>
    function viewNotificationDetails(notificationId, type) {
        fetch(`/admin/notification/${notificationId}`)  // Updated URL
            ...
            .then(data => {
                ...
                fetch(`/admin/notification/${notificationId}`, {
                    method: 'POST',
                    headers: {
